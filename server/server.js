...
const listingsDir = path.join(imagesDir, "listings")
...
if (!fs.existsSync(listingsDir)) {
  console.log("Creating listings directory")
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, uploadsDir) // Use the path variable instead of string
  },
...
app.post("/upload", authenticateToken, upload.array("images", 10), async (req, res) => {
  try {
    ...
    const ListingUploadService = require("./services/ListingUploadService")
